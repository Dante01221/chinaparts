...
export default async function GenerationCategoriesPage({ params }: { params: { brand: string; model: string; generation: string } }) {
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="bg-[#00234B] pt-20 pb-8 px-5">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center gap-2 mb-4 flex-wrap text-[11px] uppercase tracking-widest" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
            <Link href="/brands" className="text-slate-400">Каталог</Link>
            <span className="material-symbols-outlined text-slate-600 text-sm">chevron_right</span>
            <Link href={`/brands/${params.brand}`} className="text-slate-400">{brand.name}</Link>
            <span className="material-symbols-outlined text-slate-600 text-sm">chevron_right</span>
            <Link href={`/brands/${params.brand}/${params.model}`} className="text-slate-400">{carModel.name}</Link>
            <span className="material-symbols-outlined text-slate-600 text-sm">chevron_right</span>
            <span className="text-orange-400 font-bold">{gen.name}</span>
          </div>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            <span className="bg-white/10 text-white text-[11px] font-bold px-2.5 py-1 rounded-lg" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
              {gen.years}
            </span>
            <span className="text-slate-400 text-[12px]">{gen.bodyType}</span>
          </div>
        </div>
      </div>
      ...
      <main className="pb-32 px-5 max-w-2xl mx-auto mt-5">
        {availableCategories.length > 0 ? (
          <>
            <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-3" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
              Категории запчастей
            </p>
            <div className="flex flex-col gap-3">
              {availableCategories.map((cat) => {
                ...
                return (
                  <Link
                    key={cat.slug}
                    href={`/brands/${params.brand}/${params.model}/${params.generation}/${cat.slug}`}
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-12 text-center shadow-sm">
            ...
            <p className="text-slate-500 text-[15px] mb-5">Запчастей для этого поколения пока нет в каталоге.</p>
            <a
              href={`https://wa.me/996704226587?text=${encodeURIComponent(`Здравствуйте! Ищу запчасть для ${brand.name} ${carModel.name} ${gen.name} (${gen.years}).`)}`}
              target="_blank"
